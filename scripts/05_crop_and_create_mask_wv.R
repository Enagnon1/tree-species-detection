...
wv3 <- rast("../species_classification/Pansharpen.Band.tiff")
# import grid
aoi_grid <- sf::read_sf("shapefiles/processed/aoi_grid.shp")
# import tree canopy shapefile
canopy <- sf::read_sf("shapefiles/species_canopy.shp") %>% 
  sf::st_make_valid()
...
for (name in grid_with_canopy$grid_id) {
  ...
  cropped <- terra::crop(x = wv3, y = sing_grid, mask = T, snap = "out")
  ## Write cropped image
  writeRaster(cropped,
              filename = paste0(out_dir, "/", "wv3_", name, ".tif"),
              overwrite = TRUE)
  ...
  writeRaster(rasterized, 
              filename = paste0(mask_out, "/", "wv3_", name, ".tif"),
              overwrite = TRUE)
}
